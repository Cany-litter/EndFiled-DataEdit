  ...
  <div>
    <el-card>
      <div style="margin-bottom: 15px">
        <el-button type="primary" @click="dialogVisible = true">新建武器</el-button>
      </div>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑武器' : '新建武器'" width="500px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="ID"><el-input v-model="form.id" :disabled="!!editing" /></el-form-item>
        <el-form-item label="名称"><el-input v-model="form.name" /></el-form-item>
        <el-form-item label="稀有度"><el-input-number v-model="form.rarity" :min="3" :max="6" /></el-form-item>
...
async function load() { loading.value = true; list.value = (await api.get('/weapons')).data; loading.value = false }
function edit(row: any) { editing.value = row; form.value = { ...row }; dialogVisible.value = true }
function del(row: any) { api.delete(`/weapons/${row.id}`).then(load) }
async function save() {
