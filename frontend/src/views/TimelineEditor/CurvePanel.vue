    ...
    <div class="curve-scroll" :style="{ transform: 'translateX(-' + scrollLeft + 'px)' }">
      ...
      <svg :width="contentWidth" :height="totalHeight" class="curve-svg">
        ...
        <g v-for="(enemy, ei) in enemies" :key="'e'+ei">
          <text x="4" :y="staggerLabelY(ei)" font-size="9" :fill="colors[ei % colors.length]" font-weight="600">{{ lastStagger(enemy.id) }}/{{ enemy.maxStagger }}</text>
...
.context-item {
  padding: 7px 14px; font-size: 13px; cursor: pointer; transition: background 0.1s;
}
